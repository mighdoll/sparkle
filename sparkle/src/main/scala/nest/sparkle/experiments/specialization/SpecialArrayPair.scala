...
case class SpecialArrayPair[K: ClassTag, V: ClassTag](keys: Array[K], values: Array[V]) {
  self =>
  ...
  /** Apply A function that maps A key and value to A new value, and return the resulting SpecialArrayPair.
    * This is an initial attempt at providing A standard function over the SpecialArrayPair that runs fast.
    *
  ...
  def mapValues   // format: OFF 
      ...
      (fn: (K, V) => A)
      : SpecialArrayPair[K, A] = { // format: ON
    ...
    SpecialArrayPair(keys, newValues)
  }
  ...
  def mapValuesLongLongDouble// format: OFF
      (fn: (Long, Long) => Double)
      : SpecialArrayPair[Long, Double] = { // format: ON
    ...
    SpecialArrayPair(castKeys, newValues)
  }

  /** Here's a generic version, but it isn't fast, even if we specialize K and V in SpecialArrayPair.
  ...
  def mapValuesSpecialNoCast  // format: OFF
      ...
      (fn: (K, V) => C)
      : SpecialArrayPair[K, C] = { // format: ON
    val length = keys.length
    ...
    while (i < length) {
      ...
      /* this boxes. Not fast. (With K, and V specialized:)
        ...
        public nest.sparkle.core.SpecialArrayPair<K, java.lang.Object> mapValuesSpecialNoCast$mDc$sp(scala.Function2<K, V, java.lang.Object>, scala.reflect.ClassTag<java.lang.Object>);
  ...
  def mapValuesSpecialCast  // format: OFF
      ...
      (fn: (A, B) => C)
      : SpecialArrayPair[A, C] = { // format: ON
  ...
  def mapValuesDynCastSpecialize // format: OFF
    ...
    def callTypedVariant[A, B](ks: Array[A], vs: Array[B]): SpecialArrayPair[K, C] = {
      (ks, vs, classTag[C]) match {
        ...
        case (_: Array[Long], _: Array[Long], tag) if tag == classTag[Double] =>
          ...
          val result: SpecialArrayPair[Long, Double] = mapValuesSpecialCast(castFn)
          result.asInstanceOf[SpecialArrayPair[K, C]]
        ...
        case (_: Array[Long], _: Array[Long], tag) if tag == classTag[Long] =>
          ...
          mapValuesSpecialCast(castFn).asInstanceOf[SpecialArrayPair[K, C]]
