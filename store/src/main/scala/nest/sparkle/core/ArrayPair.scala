...
case class ArrayPair[K, V](keys: Array[K], values: Array[V]) {
  ...
  /** Apply A function that maps A key and value to A new value, and return the resulting ArrayPair.
    * This is an initial attempt at providing A standard function over the ArrayPair that runs fast.
    *
  ...
  def mapValues   // format: OFF 
      [@spec(Double) A: ClassTag]     // TODO recover the ClassTag at runtime by relecting on the type of Fn.
      (fn: (K, V) => A)
      : ArrayPair[K, A] = { // format: ON
  ...
  def mapValuesLongLongDouble// format: OFF
      (fn: (Long, Long) => Double)
      : ArrayPair[Long, Double] = { // format: ON
    ...
    ArrayPair(castKeys, newValues)
  }

  /** Here's a generic version, but it isn't fast, even if we specialize K and V in ArrayPair.
  ...
  def mapValuesSpecialNoCast  // format: OFF
    ...
    while (i < length) {
      ...
      /* this boxes. Not fast. (With K, and V specialized:)
  			...
  			public nest.sparkle.core.ArrayPair<K, java.lang.Object> mapValuesSpecialNoCast$mDc$sp(scala.Function2<K, V, java.lang.Object>, scala.reflect.ClassTag<java.lang.Object>);
  ...
  def mapValuesSpecialCast  // format: OFF
      [@spec(Long) A, @spec(Long) B, @spec(Double) C: ClassTag]
      (fn: (A, B) => C)
      : ArrayPair[A, C] = { // format: ON
  ...
  def mapValuesDynCastSpecialize // format: OFF
    ...
    def callTypedVariant[A, B](ks: Array[A], vs: Array[B]): ArrayPair[K, C] = {
      (ks, vs, classTag[C]) match {
        ...
        case (_: Array[Long], _: Array[Long], tag) if tag == classTag[Double] =>
          ...
          val result: ArrayPair[Long, Double] = mapValuesSpecialCast(castFn)
          result.asInstanceOf[ArrayPair[K, C]]
        ...
        case (_: Array[Long], _: Array[Long], tag) if tag == classTag[Long] =>
          ...
          mapValuesSpecialCast(castFn).asInstanceOf[ArrayPair[K, C]]
